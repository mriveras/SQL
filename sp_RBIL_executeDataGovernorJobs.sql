					...
					BEGIN 
						IF(@continue = 1)
							BEGIN
								...
								BEGIN TRANSACTION
								BEGIN TRY
									SET @sqlScript = N'EXEC dbo.sp_manageDGJobs ''' + @C_DG_jobAgentId + ''', ''EXECUTE'', @statusInt OUTPUT, @messageInt OUTPUT, @SQLInt OUTPUT, @SQLInt2 OUTPUT, @SQLInt3 OUTPUT';
											...
											----------------------------------------------------- BEGIN INSERT LOG -----------------------------------------------------
												IF(@debug = 1)
													BEGIN
														...
														SET @message      = REPLICATE(@logSpaceTree,@logTreeLevel) + 'Job (' + @C_DG_jobName + ') Executed successfully';
														SET @status       = 'Information';
														SET @SQL          = '';
														IF(@loggingType IN (1,3))
															BEGIN
